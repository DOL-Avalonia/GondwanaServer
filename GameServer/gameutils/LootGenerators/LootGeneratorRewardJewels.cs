...
{
    public class LootGeneratorRewardJewels : LootGeneratorBase
    {
        ...
        public override LootList GenerateLoot(GameObject mob, GameObject killer)
        {
            ...
            if (mob is RewardChest chest && killer is GamePlayer player)
            {
                try
                {
                    GeneratedUniqueItem generatedItem = BuildCustomMagicalItem(player, chest);
                    ...
                    if (generatedItem != null)
                    {
                        ...
                        string slotName = generatedItem.Item_Type == (int)eInventorySlot.Mythical ? "Mythirian" : "Jewellery";
                        ...
                        player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client!.Account.Language, "Lootgenerator.RewardChests.YouGetJewel", translatedName), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        SendFormattedItemNotification(player, generatedItem, rarity, slotName, translatedName);
                    }
                }
                catch (Exception e)
                {
                    log.Error($"Error generating custom jewellery for {player.Name}: ", e);
        ...
        private GeneratedUniqueItem BuildCustomMagicalItem(GamePlayer player, RewardChest chest)
        {
            ...
            List<eInventorySlot> availableSlots = new List<eInventorySlot>
            {
            ...
            if (chest.Tier == eRewardTier.RvRFinest || chest.Tier == eRewardTier.PvPTier1 || chest.Tier == eRewardTier.PvPTier2)
            {
                availableSlots.Add(eInventorySlot.Mythical);
                availableSlots.Add(eInventorySlot.Mythical);
            }

            eInventorySlot selectedSlot = availableSlots[Util.Random(0, availableSlots.Count - 1)];

            string lang = player.Client?.Account?.Language ?? LanguageMgr.DefaultLanguage;

            GeneratedUniqueItem item = new GeneratedUniqueItem(false, realm, cClass, level, eObjectType.Magical, selectedSlot, regionID, lang);

            SetJewelleryModelAndName(item, selectedSlot, chest);
            ...
            if (baseNameKey.StartsWith("[ROG]|"))
            {
                ...
                if (parts.Length >= 2) baseNameKey = parts[1];
                else if (parts.Length == 1) baseNameKey = parts[0];
            }
            else if (baseNameKey.StartsWith("[ROG]"))
            {
                string[] parts = baseNameKey.Substring(5).Split('|');
                if (parts.Length >= 3) baseNameKey = parts[2];
                else if (parts.Length == 2) baseNameKey = parts[1];
                else if (parts.Length == 1) baseNameKey = parts[0];
            }
        ...
        private void SetJewelleryModelAndName(GeneratedUniqueItem item, eInventorySlot slot, RewardChest chest)
        {
            string rarity = chest.ConfigRarityPrefix;
            eRewardTier tier = chest.Tier;

            bool isHighTier = false;
            bool isCloakTierA = false;
            bool isCloakTierB = false;
            ...
            if (tier == eRewardTier.RvRFinest)
            {
                ...
                if (rarity == "Mythical" || rarity == "Exalted") isCloakTierA = true;
                else if (rarity == "Fabled") isCloakTierB = true;
            }
            else if (tier == eRewardTier.PvPTier1)
            {
                ...
                if (rarity == "Legendary") isCloakTierA = true;
                else if (rarity == "Eminent" || rarity == "Illustrious") isCloakTierB = true;
            }

            switch (slot)
            {
                case eInventorySlot.Cloak:
                    item.Name = Util.Chance(50) ? "[ROG]|Mantle" : "[ROG]|Cloak";

                    if (isCloakTierA)
                    {
                        int[] poolA = new int[46];
                        poolA[0] = 3634;
                        for (int i = 1; i < 46; i++)
                            poolA[i] = 4603 + i; // 4604 to 4648

                        item.Model = (ushort)poolA[Util.Random(0, poolA.Length - 1)];
                    }
                    else if (isCloakTierB)
                    {
                        item.Model = (ushort)Util.Random(1720, 1727);
                    }
                    else
                    {
                        int[] otherCloakModels = new int[] { 557, 558, 559, 560, 669, 443 };
                        item.Model = (ushort)otherCloakModels[Util.Random(0, otherCloakModels.Length - 1)];
                    }
                    break;

                case eInventorySlot.Waist:
                    item.Name = Util.Chance(50) ? "[ROG]|Belt" : "[ROG]|Girdle";
                    item.Model = isHighTier ? (ushort)1884 : (ushort)597;
                    break;

                case eInventorySlot.Neck:
                    item.Name = Util.Chance(50) ? "[ROG]|Choker" : "[ROG]|Pendant";
                    ...
                    {
                        int[] neckModels = new int[] { 101, 623, 624 };
                        item.Model = (ushort)neckModels[Util.Random(0, neckModels.Length - 1)];
                    }
                    break;

                case eInventorySlot.Jewellery:
                    item.Name = Util.Chance(50) ? "[ROG]|Gem" : "[ROG]|Jewel";
                    item.Model = isHighTier ? (ushort)1886 : (ushort)Util.Random(110, 119);
                    break;

                case eInventorySlot.LeftBracer:
                case eInventorySlot.RightBracer:
                    item.Name = Util.Chance(50) ? "[ROG]|Bracelet" : "[ROG]|Bracer";
                    item.Model = isHighTier ? (ushort)1885 : (ushort)(item.Name == "[ROG]|Bracelet" ? 619 : 598);
                    break;

                case eInventorySlot.LeftRing:
                case eInventorySlot.RightRing:
                    item.Name = Util.Chance(50) ? "[ROG]|Ring" : "[ROG]|Wrap";
                    ...
                    {
                        int[] ringModels = new int[] { 103, 2520, 2521, 2522, 2523, 2524, 2525, 2526 };
                        item.Model = (ushort)ringModels[Util.Random(0, ringModels.Length - 1)];
                    }
                    break;

                case eInventorySlot.Mythical:
                    item.Name = "[ROG]|Mythirian";
                    if (tier == eRewardTier.RvRFinest)
                    {
                        item.Model = (ushort)Util.Random(4267, 4274);
                    }
                    else
                    {
                        int[] t1t2Models = new int[] { 3551, 3552, 3553 };
                        item.Model = (ushort)t1t2Models[Util.Random(0, 2)];
                    }
                    break;

                default:
                    item.Name = "[ROG]|Trinket";
                    item.Model = 101;
                    break;
            }
        }
        ...
        private void AssignValueToProp(GeneratedUniqueItem item, eProperty prop, ref double currentUti, double targetMaxUti, int targetSlots, int currentSlot)
        {
            ...
            if (remainingUti <= 0) remainingUti = 5;
        ...
        private void SendFormattedItemNotification(GamePlayer player, GeneratedUniqueItem item, string rarity, string slotName, string translatedName)
        {
