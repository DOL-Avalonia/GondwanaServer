...
namespace DOL.GS
{
    public class SchedulableServiceObjectArray<T> : ServiceObjectArray<T> where T : class, ISchedulableServiceObject
    {
        ...
        private void ScheduleInternal(ScheduleRequest request)
        {
            ...
            if (!id.TryConsumeAction(ServiceObjectId.PendingAction.Schedule) || targetTick < _lastProcessedWheelTick)
                return;
            ...
            while (_lastProcessedWheelTick <= _currentUpdateTick)
            {
                ...
                if (bucket.Count > 0)
                {
                    ...
                    for (int i = 0; i < bucket.Count; i++)
                    {
                        ...
                        if (id.SleepToken != ticket.Token || id.PeekAction() is not ServiceObjectId.PendingAction.None)
                            continue;
                ...
                if ((_lastProcessedWheelTick & WHEEL_MASK) == 0)
                {
                    while (_farFuture.TryPeek(out SleepTicket ticket, out long targetTick))
                    {
                        ...
                        if (id.SleepToken != ticket.Token || id.PeekAction() is ServiceObjectId.PendingAction.Remove)
                        {
                            _farFuture.Dequeue();
                            continue;
                        }
